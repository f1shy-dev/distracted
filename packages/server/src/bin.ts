...
function parseAgentSelection(input: unknown): AgentType[] | null {
  ...
  if (normalized === "all") return ["claude", "opencode", "pi"];
...
async function main(): Promise<void> {
  ...
  cli
    .option("--setup [agent]", "Configure AI coding agent hooks (claude, opencode, pi, all)")
    .option("--remove [agent]", "Remove AI coding agent hooks (claude, opencode, pi, all)")
  ...
  if (hasStatus) {
    ...
    UI.println(
      UI.Style.TEXT_DIM +
        `Pi:          ${status.pi ? "configured" : "not configured"}` +
        UI.Style.TEXT_NORMAL,
    );
    UI.empty();
    process.exit(0);
  ...
  if (hasSetup) {
    ...
    if (agents && agents.length === 0) {
      UI.error("Invalid agent. Use: claude, opencode, pi, all");
      process.exit(1);
    }
  ...
  const status = await getSetupStatus();
  if (!status.claude && !status.opencode && !status.pi) {
    UI.println(
